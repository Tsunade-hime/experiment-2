...
class C2fMSBlockExp(C2f):
    """
    C2f + MSBlock
    保留 C2f 外层框架，仅把内部重复块从 Bottleneck 替换成 MSBlock
    """
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5,
        ...
        参数:
            ...
            branches                  : MSBlock 分支数
            k                         : MSBlock 内部分支卷积核大小
            expand_ratio              : MSBlock 内部分支扩张比例
            act                       : MSBlock 是否使用激活

        说明:
            g 在 MSBlock 中不使用，这里保留只是为了与 C2f 系列接口统一。
        """
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            MSBlock(c1=self.c, c2=self.c, branches=branches, k=k,
                    expand_ratio=expand_ratio, shortcut=shortcut, act=act)
            for _ in range(n))
