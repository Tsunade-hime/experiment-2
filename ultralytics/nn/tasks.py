...
from .SPPmodules import *
from .Convmodules import *
...
def parse_model(d, ch, verbose=True):
    ...
    attn_modules = frozenset({SE, ECA, CA, SimAM, SK, BAM, A2, SLAM,
                              TripletAttention, CBAM, GAM, SCSA, EMA, ACmix, ELA, BinaryAttention, Wave2D, DHOGSA, ASSA,
                              CirculantAttention, MHLA, GLFA})  # 逐步加
    ...
    conv_repeat_modules = frozenset({C3GhostExp, C3XExp, C3TRExp, HGBlockExp, C3SCConvExp,
                                     C3FasterBlockExp, C2fMSBlockExp, C2fHGBlockExp})
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ✅ 在这里加：变种卷积模块（输入输出通道不变，但构造函数需要 channels）
        ...
        # ✅ 在这里加：SPP模块（输入输出通道不变，但构造函数需要 channels）
